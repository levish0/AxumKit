...
/// Whether an older comment (smaller uuidv7 id) exists in the same thread scope.
pub async fn repository_exists_older_board_comment<C>(
    conn: &C,
    post_id: Uuid,
    ...
    C: ConnectionTrait,
{
    let mut query = CommentEntity::find()
        ...
        .filter(CommentColumn::Id.lt(cursor_id));
