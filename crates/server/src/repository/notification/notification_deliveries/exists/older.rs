...
/// Checks whether a notification older than the current cursor exists.
///
/// # Role
...
pub async fn repository_exists_older_notification<C>(
    conn: &C,
    user_id: Uuid,
    ...
    C: ConnectionTrait,
{
    let query = apply_notification_filter(
        NotificationEntity::find()
            ...
            .filter(NotificationColumn::Id.lt(cursor_id)),
        filter,
    );
