...
use crate::service::board::service_lock_board_post;
...
#[utoipa::path(
    post,
    path = "/v0/board/post/lock",
    summary = "Lock a board post",
    description = "Locks the post so no new comments can be added. Requires board moderation permission.",
    request_body = BoardPostModerationRequest,
    responses(
        (status = 200, description = "Post locked successfully", body = BoardPostModerationResponse),
...
pub async fn lock_post(
    ...
    service_lock_board_post(&state.db, payload, &session).await
}
