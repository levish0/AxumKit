...
use crate::service::board::service_pin_board_post;
...
#[utoipa::path(
    post,
    path = "/v0/board/post/pin",
    summary = "Pin a board post",
    description = "Pins the post to the top of its board. Requires board moderation permission.",
    request_body = BoardPostModerationRequest,
    responses(
        (status = 200, description = "Post pinned successfully", body = BoardPostModerationResponse),
...
pub async fn pin_post(
    ...
    service_pin_board_post(&state.db, payload, &session).await
}
