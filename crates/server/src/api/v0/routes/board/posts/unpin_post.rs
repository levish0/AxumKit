...
use crate::service::board::service_unpin_board_post;
...
#[utoipa::path(
    post,
    path = "/v0/board/post/unpin",
    summary = "Unpin a board post",
    description = "Removes the pinned state from the post. Requires board moderation permission.",
    request_body = BoardPostModerationRequest,
    responses(
        (status = 200, description = "Post unpinned successfully", body = BoardPostModerationResponse),
...
pub async fn unpin_post(
    ...
    service_unpin_board_post(&state.db, payload, &session).await
}
