...
use crate::service::acl::service_list_acl_group_members;
...
#[utoipa::path(
    ...
    params(ListAclGroupMembersRequest),
    responses(
        (status = 200, description = "ACL group members retrieved successfully", body = AclGroupMemberListResponse),
...
pub async fn list_acl_group_members(
    State(state): State<AppState>,
    RequiredSession(session): RequiredSession,
    ValidatedQuery(payload): ValidatedQuery<ListAclGroupMembersRequest>,
) -> Result<AclGroupMemberListResponse, Errors> {
    service_list_acl_group_members(&state.db, payload, &session).await
}
