...
use crate::service::acl::membership::{self, Authority, GrantParams, MemberSubject};
...
///
/// # System groups
/// - Refused (`Errors::AclGroupIsSystem`) via the membership chokepoint:
///   system groups carry code-known meaning and are not mutable through the
///   generic admin API.
///
/// # Errors
/// - Returns `Errors::AclInvalidRule` unless exactly one subject is given or
///   when `expires_at` is in the past.
/// - Returns `Errors::AclGroupNotFound` / `Errors::AclGroupIsSystem` /
///   `Errors::UserNotFound` / `Errors::InvalidIpAddress` for invalid targets.
/// - Returns `Errors::AclGroupMemberAlreadyExists` for an active duplicate.
pub async fn service_add_acl_group_member(
    db: &DatabaseConnection,
    payload: AddAclGroupMemberRequest,
    ...
    if payload.user_id.is_some() == payload.ip_address.is_some() {
        return Err(Errors::AclInvalidRule(
    ...
    repository_create_moderation_log(
        &txn,
        ModerationAction::AclGroupMemberAdd,
