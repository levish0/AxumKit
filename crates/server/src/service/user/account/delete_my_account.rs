...
/// Result of an account-deletion request: either the account was deleted inline (after an inline
/// re-authentication factor), or a confirmation email was sent (OAuth-only accounts with no
/// inline factor).
...
///
/// Account deletion is irreversible, so a stolen cookie or XSS-driven request must not be able to
/// trigger it alone (OWASP ASVS 7.5.1). The re-authentication factor is chosen from the account:
...
pub async fn service_request_account_deletion(
    ...
    redis: &RedisClient,
    worker: &crate::state::WorkerClient,
    ...
    if let Some(password_hash) = user.password.as_deref() {
        ...
        perform_account_deletion(db, redis, worker, user).await?;
        ...
        // Store under the hashed token id so the raw token never lives in Redis; the raw token is
        // returned and only ever sent in the confirmation email.
        ...
        worker_client::send_account_deletion(
            worker,
            &user.email,
            &user.handle,
            &token,
...
///
/// The user row is hard-deleted; FK cascades remove OAuth connections, roles, bans, and known
/// devices. Sessions and the search-index entry are then cleaned up best-effort — those failures
/// are logged but never resurrect the account.
async fn perform_account_deletion(
    ...
    repository_delete_user(db, user_id).await?;
    ...
    if let Err(e) = worker_client::delete_user_from_index(worker, user_id).await {
        warn!(user_id = %user_id, error = ?e, "Failed to queue search-index removal after account deletion");
    }
