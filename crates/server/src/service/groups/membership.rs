...
use crate::repository::group_members::{
    ...
    repository_delete_group_members_for_ip, repository_delete_group_members_for_user,
    repository_find_active_group_member_for_ip, repository_find_active_group_member_for_user,
};

/// A membership subject: a user or an IP/CIDR — exactly one, mirroring the DB
/// CHECK on `group_members`.
#[derive(Clone, Copy)]
...
async fn find_active<C: ConnectionTrait>(
    ...
    match subject {
        MemberSubject::User(user_id) => {
            repository_find_active_group_member_for_user(conn, group_id, user_id).await
        }
        MemberSubject::Ip(ip) => {
            repository_find_active_group_member_for_ip(conn, group_id, &ip).await
        }
    }
}
...
async fn delete_for_subject<C: ConnectionTrait>(
    ...
    match subject {
        MemberSubject::User(user_id) => {
            repository_delete_group_members_for_user(conn, group_id, user_id).await?;
        }
        MemberSubject::Ip(ip) => {
            repository_delete_group_members_for_ip(conn, group_id, &ip).await?;
        }
    }
    Ok(())
}
